...
- 工具：矩形圈选、自由画笔、箭头；自动编号 ①②③；三色（红/黄/蓝）；撤销/清空；1x/2x 放大；
...
- Agent 在高保真探索时可带 `fastPreview` 参数先出**方向稿**（面板「方向稿模型」档，快模型）；
...
| 标注反馈是什么                    | 点生成图进标注弹窗，矩形/画笔/箭头圈选要改的区域（自动编号 ①②③），与意见一并提交；编号归一化坐标随消息发出，「改哪里」不依赖措辞（见 §5.4）                                                           |
