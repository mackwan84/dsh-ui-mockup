...
- 工具：默认“抓手”拖动画布；切换矩形、自由画笔、箭头后绘制；自动编号 ①②③；三色（红/黄/蓝）；缩放下拉提供“适合窗口”（默认，完整显示且不超过100%）、50%、100%、150%、200%；
- 编辑已有标记：抓手模式单击矩形内部或画笔/箭头线段附近即可单选，选中项显示虚线框；**点过画布后**按Delete或macOS Backspace删除，删除与清空都可用“撤销”恢复；在意见输入框或工具条按钮上按删除键不会误删标记；
...
| 标注反馈是什么                    | 点生成图进标注弹窗；默认抓手可拖动画布、选择并删除已有标记，切换矩形/画笔/箭头后圈选区域（自动编号 ①②③），与意见一并提交；编号归一化坐标随消息发出（见 §5.4）                                         |
