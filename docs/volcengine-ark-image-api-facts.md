...
- **尺寸迁移约束**：不能继续发送 SeedEdit 的 `adaptive`。2026-08-28 真实调用结果为：
  ...
  显式 `2048x1152` 也成功。
- **编辑路径显式像素下限低于生成路径（实测）**：`2048x1152` 总像素 2,359,296，
  低于生成路径实测下限 3,686,400，编辑仍成功出图——因此编辑路径**不套用**生成的
  钳制规则，显式值仅归一格式后透传。更低尺寸（如偏好下拉的 `1024*1024`）在编辑
  路径是否被网关接受**未确认**；若被拒绝，错误经错误映射如实上报，用户改用
  `2K` 档位或更大尺寸即可恢复。
