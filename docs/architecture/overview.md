...
| M5 ✅  | 标注弹窗（抓手平移、矩形/画笔/箭头、标记选择删除 + 编号坐标文本反馈）+ `fastPreview` 方向稿 + 方向稿模型档 + 精修按钮 + 历史方向稿字段/过滤 | 标注交互与几何回归 + fastPreview 组合语义/设锚提示组合测试通过 |
