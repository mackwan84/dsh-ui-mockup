...
- **设置面板 4 页**（已确认线框）：概览 / 提供方与模型（三列提供方选择、统一连接设置与三档模型默认）/ 生成偏好 / 生成历史（方向稿标签 + 「只看方向稿」服务端过滤，工作区/连接变化时过滤位与请求参数一同复位）；
  0.3.0 起模型分层改为组合框（input + datalist）：候选 = 注册表静态 hints ∪ `provider/models` 网关拉取建议（仅 openai-compat 拉取；失败/未配置/HTML-200 一律静默降级为空），永远可手填任意模型名；openai-compat 在连接设置区额外显示网关地址行（baseUrl 编辑经 `provider/baseurl/set` 写用户层补丁 config 节并热重载落位，restates 生效配置全部键）；凭据的其他配置方式默认折叠，减少首屏说明占用；
