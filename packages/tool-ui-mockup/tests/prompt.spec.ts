...
describe('buildPrompt', () => {
  it('renders a Balsamiq-style wireframe template with Chinese annotation guidance', () => {
    const prompt = buildPrompt({ description: '待办事项主页', fidelity: 'wireframe', platform: 'web' }, false)
  ...
  it('falls back to a default style when none is given', () => {
    const prompt = buildPrompt({ description: 'x', fidelity: 'high-fidelity', platform: 'web' }, false)
    expect(prompt).toContain('极简浅色, 现代简洁')
  })

  it('demands reference consistency in I2I mode', () => {
    const prompt = buildPrompt({ description: '搜索结果页', fidelity: 'high-fidelity', platform: 'web' }, true)
