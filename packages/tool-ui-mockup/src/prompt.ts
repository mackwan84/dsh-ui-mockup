...
export function buildPrompt(args: PromptArgs, hasReference: boolean): string {
  ...
  if (args.fidelity === 'wireframe') {
    return `黑白线框图(wireframe)UI 草图, 类似 Balsamiq 手绘风格。只用灰白色块、细边框与简单占位图形表示元素, 无配色、无阴影、无渐变、无照片。每个区块内用清晰的中文小字标注用途, 例如"导航栏"、"搜索框"、"商品卡片"、"按钮"。区块内不写大段正文: 标题与按钮用简短常见词, 列表与卡片内容用短语占位(如"商品名称"、"¥99"), 避免长句与生僻字。完整展示页面布局、信息层级与主要交互入口。${base}`
  }
  const style =
    args.style !== undefined && args.style.trim() !== '' ? args.style.trim() : '极简浅色, 现代简洁'
  return `高保真 UI 界面设计稿, 设计精细, 整体采用${style}的风格。所有中文文案必须准确、清晰、无乱码、排版专业。正文与列表内容用简短常见短语占位, 避免大段密集长文本。配色统一、字体层级分明、组件状态完整、间距合理。${base}`
}
