...
export function AnnotateModal({ name, cwd, t, onClose, onSubmit }: AnnotateModalProps) {
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    const focusables = Array.from(
      root.querySelectorAll<HTMLElement>('button, [href], textarea, select, input'),
  ...
  }, [marks, draftMark, zoom, imageSize, color])
  ...
  const onPointerUp = () => {
    ...
    if (isCommitable(draftMark)) {
      setMarks((prev) => [...prev, { mark: draftMark, color }])
    }
    setDraftMark(null)
  }

  // 1x/2x 切换：保持当前视口中心在放大后仍居中（提案的「以点击位置为中心」）
  const toggleZoom = () => {
    const next: ZoomLevel = zoom === 1 ? 2 : 1
    ...
    >
      <div style={panelStyle}>
        ...
        >
          ...
          <span style={{ flex: 1 }} />
          {(['rect', 'brush', 'arrow'] as const).map((item) => (
            <Button
              key={item}
          ...
          <Button variant="ghost" size="sm" onClick={toggleZoom} disabled={imageSize === null}>
            {zoom === 1 ? t('annotate.zoomIn') : t('annotate.zoomOut')}
          </Button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Button variant="primary" size="sm" disabled={opinion.trim() === ''} onClick={submit}>
