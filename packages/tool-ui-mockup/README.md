...
- 标注反馈（0.2.0）：卡片图片点击进标注弹窗（默认抓手拖动画布、矩形/画笔/箭头、已有标记单选与画布聚焦时 Delete/Backspace 删除、可撤销删除/清空（栈深 50）、自动编号、适合窗口与50%/100%/150%/200%缩放），提交时编号
...
- 标注弹窗的圈选绘制不支持键盘操作；画布可聚焦，聚焦后已选标记可用Delete/Backspace删除（输入控件与工具条按钮持有焦点时不删）；无障碍兜底仍为意见输入框 + 提交按钮单独完成纯文字反馈。
